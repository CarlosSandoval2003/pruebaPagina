...
const Shop = () => {
  ...
  const fetchPromoCategory = async () => {
    try {
      const response = await fetch('http://181.209.233.54:4000/promo-category');
  ...
  const fetchInfo = () => {
    fetch('http://181.209.233.54:4000/popularinwomen')
      .then((res) => res.json())
      .then((data) => setPopular(data));

    // Modifica la URL para obtener los productos más recientes desde Express.js
    fetch('http://181.209.233.54:4000/productos-recientes') // Cambia esta URL
      .then((res) => res.json())
