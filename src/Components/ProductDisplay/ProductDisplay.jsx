...
const ProductDisplay = (props) => {
  ...
  useEffect(() => {
    ...
    fetch(`http://192.168.1.20:4000/firstOption/${props.id}`)
  ...
  useEffect(() => {
    ...
    if (selectedOption1) {
      if (tipos.length > 1) {
        fetch(`http://192.168.1.20:4000/secondOption/${selectedOption1}/${props.id}`)
          .then(response => response.json())
          .then(data => {
        ...
        const opcion2 = "0";
        fetch(`http://192.168.1.20:4000/configurationProduct/${selectedOption1}/${opcion2}/${props.id}`)
          .then(response => response.json())
          .then(data => {
  ...
  const handleSelectOption2 = (e) => {
    ...
    fetch(`http://192.168.1.20:4000/checkAvailability/${props.id}/${selectedValue}/${selectedOption1}`)
      .then(response => response.json())
      .then(data => {
        if (data.available) {
  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      const cartResponse = await fetch(`http://192.168.1.20:4000/cart/${userId}`);
      const cartData = await cartResponse.json();

      let cartId;
      // Si no hay un carrito existente, crear uno nuevo
      if (!cartData.cartExists) {
        const newCartResponse = await fetch(`http://192.168.1.20:4000/cart`, {
          method: "POST",
          headers: {
    ...
    console.log("TIPOS EN ADD CARRITO:",tipos);
      ...
      if (tipos.length > 1) {
        const configurationResponse = await fetch(`http://192.168.1.20:4000/configurationProduct/${selectedOption1}/${selectedOption2}/${props.id}`);
      ...
      const cartItemResponse = await fetch(`http://192.168.1.20:4000/cartItem/${cartId}/${itemId}`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (cartItemData.success) {
        ...
        const configurationResponse = await fetch(`http://192.168.1.20:4000/configurationProduct/${selectedOption1}/0/${props.id}`);
        const configurationData = await configurationResponse.json();
        const itemId = configurationData.itemId;
        console.log(itemId, cartId);
  ...
  useEffect(() => {
    if (selectedOption1 && selectedOption2) {
      fetch(`http://192.168.1.20:4000/configurationProduct/${selectedOption1}/${selectedOption2}/${props.id}`)
        .then(response => response.json())
        .then(data => {
          setPrice(data.price); // Establecer el precio del producto
        })
