...
const ItemProducto = mongoose.model("ITEM_PRODUCTO", {
  id_producto: {
    type: Number,
    required: true,
  },
...
const Variacion = mongoose.model("VARIACION", {
  nombre: {
    type: String,
    required: true,
  },
...
const OpcionVariacion = mongoose.model("OPCION_VARIACION", {
  id_variacion: {
    type: Number,
    required: true,
  },
...
const ConfiguracionProducto = mongoose.model("CONFIGURACION_PRODUCTO", {
  id_item_producto: {
    type: Number,
    required: true,
  },
...
app.get('/productos-recientes', async (req, res) => {
  ...
  try {
    ...
    // Ejecuta la consulta para obtener los 5 productos más recientes
    const result = await connection.execute(
      `SELECT * FROM (
        SELECT p.id, p.nombre_producto, p.descripcion_producto, p.imagen_producto1, c.nombre_categoria
        FROM PRODUCTO p
...
app.post("/addpromotions", async (req, res) => {
  ...
  const newPromotionCategoryData = {
    id_categoria: req.body.id_categoria,
    id_promocion: null // Se actualizará después de insertar la promoción
  };
  ...
  try {
    ...
    res.status(500).json({ success: false, error: "Error al agregar la promoción" });
  } finally {
    if (connection) {
      try {
        await connection.close();
